...
export function generate() {
  ...
  try {
    ...
    const data1 = labels.map(label => dist1[label] == 0 ? NaN : dist1[label] || NaN);
    const data2 = labels.map(label => dist2[label] == 0 ? NaN : dist2[label] || NaN);
    console.log("Final labels and data:", { labels, data1, data2 });

    updateChart(labels, [
      { label: groups1.map(g => g.raw).join(""), data: data1, borderColor: 'rgba(54, 162, 235, 0.6)', backgroundColor: 'rgba(54, 162, 235, 0.2)' },
      { label: groups2.map(g => g.raw).join(""), data: data2, borderColor: 'rgba(255, 99, 132, 0.6)', backgroundColor: 'rgba(255, 99, 132, 0.2)' }
    ]);
  } catch (error) {
